...
def dp_vod_list(p):
    ...
    if 'list' in items:
        for item in items['list']:
            ...
            if title:
                if epno not in title: title += ' %s회' % epno
            else: title = '%s회' % epno
            plot = item['synopsis'].encode('utf8').replace('<br>', '').replace('</br>', '').replace(
                              '<b>', '[B]').replace('</b>', '[/B]') if 'synopsis' in item else item['episodetitle'].encode('UTF-8')
    ...
    if quality_idx is not None:
        ...
        if p['category'] == 'VOD_Sub_List':
            ...
            if code == programitems['contentid']:
                ...
                plot = programitems['synopsis'].encode('utf8').replace('<br>', '').replace('</br>', '').replace(
                    '<b>', '[B]').replace('</b>', '[/B]')
        elif p['category'] == 'Movie_Sub_List':
            movieitems = Wavve().GetMovieInfo(code)
            plot = movieitems['synopsis'].encode('utf8').replace('<br>', '').replace('</br>', '').replace(
                '<b>', '[B]').replace('</b>', '[/B]')
        else: plot = ''
        if 'img' in p:
...
def addDir(item, infoLabels=None):
    ...
    try:
        ...
        item['title'] = quote(item['title'].encode('UTF-8'))
